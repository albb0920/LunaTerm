...
public abstract class vt320 extends VDUBuffer implements VDUInput {
	...
	public void putString(String s, String encoding) {
		String cns = "";
		try {
			cns = new String(s.getBytes("ISO8859-1"), encoding);
		...
		if (len > 0) {
			...
			for (int i = 0; i < len; i++) {
				...
				byte _b = (byte) c;
				if (_b > 0 && preByte < 0) {
					if (ChineseUtils.isCn(cns, new byte[] { preByte, _b },
							encoding))
						c = (char) (_b - 2009);
				}

				// if (_b == 27 && (int) s.charAt(i) != (int) c) {
				// if (R==13 && i == len - 1 && _b == 27) {
				// for (int t = 0; t < s.length(); t++) {
				// System.out.println("ESC(" + R + "):"
				// + (byte) s.charAt(t));
				//
				// }
				// System.out.println("ESC@:" + (int) s.charAt(i) + ","
				// + (int) c);
				// }
				preByte = (byte) s.charAt(i);
	 ...
	 * System.out.println("keyReleased("+evt+")"); // ignore }
	 */
	/**
